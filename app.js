...
/* ══════════════════════════════════════
   ...
   USA thumbnail?id= que funciona cross-origin
...
function normalizeDriveUrl(url) {
  ...
  // Já está no formato uc?export=view&id=ID — extrai o ID
  ...
  if (fileId) {
    // thumbnail?id= funciona sem CORS e sem bloqueio do Drive
...
function initHeroReveal() {
  ...
  const wrapWords = (el) => {
    el.childNodes.forEach(node => {
      ...
      } else if (node.nodeName === 'EM') { wrapWords(node); }
    });
  };
  wrapWords(title);
